...
{
    public class ISO12233Chart : AbstractTestChart
    {
        public override string ChartName { get { return "ISO12233 Chart"; } }
        public override string ChartMemo { get { return "ISO Standard chart"; } }

        private float ResoveBrightPercent=73.5f;//分辨率的默认数值，这个数值被修改，测试的分辨率就会改变

        public ISO12233Chart(WriteableBitmap Photo)
        {
            //InitBWList();
            //InitColorList();
            //ChartPhoto = Photo;
        ...
        public ISO12233Chart() { }
        //public override void setChart(WriteableBitmap b)
        //{
        //    base.setChart(b);
        //    //CorrectISO12233Chart();
        //}
        ...
        public long getLPResoveLines()//自动从原始的照片
        {
            ...
            WriteableBitmap xb = getAreaLResove(ChartPhoto);//这里似乎是从原始照片而不是校准照片开始的
            ...
            if (l == 0||l>1900)//如果左侧的测试没有测出一个可以的成绩或者成绩大于1900
            {
                //xb.Dispose();
        ...
        {//主要是需要判读这些曲线属于哪个级别的
            ...
            if ((ln > 6) && (!IsLeft) && (ln < 9))//说明是老的ISO12233高分辨测试
            {
                ...
                //setProcessInfor("ISOCardType", "1000-4000");
        ...
        public decimal getVDispersiveness()
        {
            ...
            if (mp.CorrectPhoto == null) { return -1; }
            WriteableBitmap cb = getAreaVEdge(mp.CorrectPhoto);
        ...
        {
            WriteableBitmap b = getAreaVEdge(mp.CorrectPhoto);//这里找到的是水平的边界，因此裁剪的时候垂直进行
        ...
        public int getHEdgeResoveLines()//得到分辨率，使用线扩散函数计算
        {
            ...
            WriteableBitmap b = getAreaHEdge(mp.CorrectPhoto);
            //b=AutoBright(b);
            int x = ptp.getEdgeResoveLines(b, true, mp.SourcePhoto.PixelHeight);
            return x;
        }

        public int getVEdgeResoveLines()//得到分辨率，使用线扩散函数计算
        {
            WriteableBitmap b = getAreaVEdge(mp.CorrectPhoto);
            //b=AutoBright(b);
            int x = ptp.getEdgeResoveLines(b, false, mp.SourcePhoto.PixelHeight);
            return x;
        }
        ...
        {
            //int sx = Convert.ToInt32(b.PixelHeight / 9 * 16 * 305 / 535d);
            //sx = getCenterX(sx, b.PixelWidth, b.PixelHeight);
            //int sy = Convert.ToInt32(b.PixelHeight * 251 / 356d);
            //int h = Convert.ToInt32(b.PixelHeight * 18 / 356d);
            //int w = Convert.ToInt32(b.PixelHeight * 90 / 356d);
            //WriteableBitmap tb = getImageArea(b, sx, sy, w, h);
            //if (IsAnalyse) { DrawSelectArea(sx, sy, w, h); }
            //setSelectArea("AreaHEdge", tb);

            int cx = b.PixelWidth / 2;//中央x
            int cy = b.PixelHeight / 2;//中央y
            int hhw = Convert.ToInt32( 146d / 2856d * b.PixelHeight);//下方选择区的宽度一半
            int hh = Convert.ToInt32(80d / 2856d * b.PixelHeight);//下方选择区的高度
            int ht = Convert.ToInt32(1832d / 2856d * b.PixelHeight);//先锋选择区的起始高度

            WriteableBitmap tb = getImageArea(b, cx-hhw, ht, hhw*2, hh);
            if (IsAnalyse) { DrawSelectArea(cx - hhw, ht, hhw * 2, hh); }
            setSelectArea("AreaHEdge", tb);
        ...
        {
            //int pw = b.PixelHeight / 356 * 535;//计算出来的全尺寸宽度
            //int sx = Convert.ToInt32(296/535*pw)-(pw-b.PixelWidth)/2;
            //sx = getCenterX(sx, b.PixelWidth, b.PixelHeight);
            //int sx = Convert.ToInt32(b.PixelWidth * 2055 / 4280);
            //sx = getCenterX(sx, b.PixelWidth, b.PixelHeight);

            int cx = b.PixelWidth / 2;//中央x
            int cy = b.PixelHeight / 2;//中央y
            int hvw = Convert.ToInt32(40d / 2856d * b.PixelHeight);//下方选择区的宽度一半
            int vh = Convert.ToInt32(280d / 2856d * b.PixelHeight);//下方选择区的高度
            int vt =Convert.ToInt32(690d / 2856d * b.PixelHeight);//先锋选择区的起始高度

            WriteableBitmap tb = getImageArea(b, cx - hvw, vt, hvw * 2, vh);
            if (IsAnalyse) { DrawSelectArea(cx - hvw, vt, hvw * 2, vh); }
