...
{
    public class LynxParentLine : LynxDesignObject, IDesignRelation, IObjectMonitor//不含一个数据对象，可以被记录的，关联关系被记录在外键里面
    {//只能对特定类型进行依赖，采用特定线形表示隶属关系，用来强制表示一种父子依赖
        #region Constructor
        public LynxParentLine()
        {
        ...
        Polyline ArrowPolyLine = new Polyline();
        ...
        public override void ClearAllEvent()
        {
            ...
            //if (eb!= null)
            //{
            //    eb.LostFocus -= new RoutedEventHandler(eb_LostFocus);
            //}
        }//需要子类实现
        #endregion
        ...
        public override void Active()
        {
            ...
            ArrowPolyLine.Stroke = DesignObjectActiveBrush;
            tb.Foreground = DesignObjectActiveBrush;
            Canvas.SetZIndex(MainLine, Canvas.GetZIndex(this) + 1);
            Canvas.SetZIndex(ArrowPolyLine, Canvas.GetZIndex(this) + 2);
        ...
        public override void DeActive()
        {
            ...
            ArrowPolyLine.Stroke = DesignObjectBackBrush;
            tb.Foreground = DesignObjectBackBrush;
            Canvas.SetZIndex(MainLine, Canvas.GetZIndex(this));
            Canvas.SetZIndex(ArrowPolyLine, Canvas.GetZIndex(this)+1);
        ...
        public void RemoveRelationLine() 
        {
            ...
            if (designCanvas.getCanvas().Children.Contains(ArrowPolyLine)) { designCanvas.getCanvas().Children.Remove(ArrowPolyLine); }
        ...
        void MoveArrowLine()//依据新的位置转动箭头
        {
            ...
            ArrowPolyLine.Fill = Background;
            ArrowPolyLine.Points.Clear();//重建，否则会重复添加
            ArrowPolyLine.Points.Add(new Point(ex1, ey1));
            ArrowPolyLine.Points.Add(EndPoint);
            ArrowPolyLine.Points.Add(new Point(ex2, ey2));
            ArrowPolyLine.Points.Add(new Point(ex1, ey1));
        }
        ...
        void tb_PointerPressed(object sender, PointerRoutedEventArgs e)
        {
            TextBlock tb = sender as TextBlock;
            TextBox eb = new TextBox();
            eb.Text = tb.Text;
            Canvas.SetLeft(eb, Canvas.GetLeft(tb));
            Canvas.SetTop(eb, Canvas.GetTop(tb));
            Canvas.SetZIndex(eb, Canvas.GetZIndex(tb) + 1);
            designCanvas.getCanvas().Children.Add(eb);
            eb.LostFocus += new RoutedEventHandler(eb_LostFocus);
        }

        void eb_LostFocus(object sender, RoutedEventArgs e)
        {
            TextBox eb = sender as TextBox;
            Name = eb.Text;
            tb.Text = eb.Text;
            eb.LostFocus -= new RoutedEventHandler(eb_LostFocus);
            designCanvas.getCanvas().Children.Remove(eb);
        }
        ...
        void DrawLineCap(Point StartPoint, double LineArc)//从某点开始以约定的长度和角度绘制箭头
        {
            ...
            {
                ArrowPolyLine = new Polyline();
            }

            ArrowPolyLine.StrokeThickness = ArrowLineWidth;
            ArrowPolyLine.Stroke = new SolidColorBrush(ForeColor);
            ArrowPolyLine.Fill = Background;
            ...
            if (!designCanvas.getCanvas().Children.Contains(ArrowPolyLine))
            {
                designCanvas.getCanvas().Children.Add(ArrowPolyLine);
            }
