...
{
    public partial class SFRResolution : UserControl
    {
        ...
        {
            if ( b == null) { return; }
            //this.SourceImage.Source = b;
            lChartPhoto1.setPhoto(b);
            lChartPhoto1.getImage().PointerPressed += new PointerEventHandler(image_PointerPressed);
        ...
        void image_PointerPressed(object sender, PointerRoutedEventArgs e)
        {
            try
            {
                ...
                Point? p = ptp.PointToPix(lChartPhoto1.getPhoto(), lChartPhoto1.getImage(), e.GetCurrentPoint(lChartPhoto1.getImage()).Position, true);
                if (p == null) { return; }
                double pp;
                if (IsV)
                {
                    pp = p.Value.Y / (lChartPhoto1.getPhoto()).PixelHeight;
                }
                else
                {
                    pp = p.Value.X / (lChartPhoto1.getPhoto()).PixelWidth;
                }
                ...
                if (IsV)
                {
                    ...
                    LSF = SilverlightLFC.common.Environment.getDoubleList<decimal>(ptp.getDdx(EdgeBrightList));
                    al = ptp.getCurveMTF(lChartPhoto1.getPhoto(), false, pp);
                }
                else
                {
                    ...
                    al = ptp.getCurveMTF(lChartPhoto1.getPhoto(), true, pp);
                }
                edg.DrawLines(EdgeBrightList);
                ldg.DrawLines(LSF);
